	...
	if(isset($device_id)){
		...
		if($tokenFound[$TAG_SUCCESS]){
			...
			if($req){	
				...
				//if($user_id !=0)
				{
					$user_status = getNurseStatus($db_con, $user_id);
...
function sendMentionNotification($db_con,$mentionUserId,$commentId){

	include("../include/constant.php");
	$userName='';

	$query = "SELECT * FROM `$CONTENT_COMMENT_TABLE` WHERE `$CONTENT_COMMENT_ID` = '$commentId'";
	$result = mysqli_query($db_con,$query);
	if($result){
		if(mysqli_num_rows($result) != 0){
			$rowData = mysqli_fetch_array($result);
			$userData=getNurse($db_con,$rowData['Login_User_ID']);

			$userName=$userData['first_name'];
		}
	}

	if ( $userName !='' && $mentionUserId !='' ) {
		...
		$query	= "SELECT gcm_token FROM `$USER_DEVICE_TABLE` WHERE `$USER_DEVICE_USER_ID` = '$mentionUserId'";
		$result = mysqli_query($db_con,$query);
		if ($result) {
			if(mysqli_num_rows($result) != 0){
				while ($row1 = mysqli_fetch_array($result)) 
				{
					$registrationIds[]=$row1['gcm_token'];
				}
		...
		if (!empty($registrationIds)) {
			include('../../GCM.php');
			$gcm = new GCM();
			$gcm->send_mentioned_notification($registrationIds, $msg);	
...
/* getNurseUserRow
*/
function getNurseUserRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();							
	$detail[$TAG_ID]				= $row[$NURSE_ID];
	$detail[$TAG_NAME]			= $row[$NURSE_FIRST_NAME].'_'.$row[$NURSE_LAST_NAME];
	$detail[$TAG_IMAGE]		= setImagePath($NURSE_PATH_TYPE, $row[$NURSE_PROFILE_IMAGE]);
...
function getNurseRow($db_con,$row) {
	...
	$query = "SELECT device_id FROM `$USER_DEVICE_TABLE` WHERE `$USER_DEVICE_USER_ID` = '$row[$NURSE_ID]'";
	$result = mysqli_query($db_con,$query);
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row_new = mysqli_fetch_array($result);
			$device_id=$row_new['device_id'];
			
			
		}
	}

	$detail[$USER_DEVICE_DEVICE_ID] =$device_id ;
	
	$_district_id				= $row[$NURSE_DISTRICT_ID];
	$detail[$TAG_LOCATION]				= getLocation($db_con, $_district_id);
...
function getNurseProfileRow($db_con,$row) {
    ...
    $detail[$TAG_TRAINER]            = $row[$NURSE_TRAINER];
	...
	$_district_id				= $row[$NURSE_DISTRICT_ID];
	$_hospital_condition_ids	= $row[$NURSE_HOSPITAL_CONDITION_ID];
	
	$detail[$TAG_HOSPITAL]				= getHospital($db_con,$_hospital_id);
	$detail[$TAG_HOSPITAL_CONDITION]	= getHospitalCondition($db_con,$_hospital_condition_ids);
...
function getHospitalRow($db_con,$row) {
	...
	$_hospital_id			= $row[$HOSPITAL_ID];
	$detail[$TAG_PARTNER]	= getHospitalPartner($db_con,$_hospital_id);
	//$detail[$TAG_EDIT_TIME]		= $row[$HOSPITAL_EDIT_TIME];
	
	return $detail;
}

/* getHospitalPartnerRow
*/
function getHospitalPartnerRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();							
	$detail[$TAG_ID]					= $row[$HOSPITAL_PARTNER_ID];
	$detail[$TAG_NAME]				= $row[$HOSPITAL_PARTNER_NAME];
	$detail[$TAG_STATUS]			= $row[$HOSPITAL_PARTNER_STATUS];
	$detail[$TAG_ENTRY_TIME]	= $row[$HOSPITAL_PARTNER_ENTRY_TIME];
	return $detail;
}
...
/* getLikeRow
*/
function getLikeRow($db_con,$row) {
	
	include("../include/constant.php");	
	
	$detail = array();							
	$detail[$TAG_ID]				= $row[$CONTENT_LIKE_ID];
	$detail[$TAG_CONTENT_ID]		= $row[$CONTENT_LIKE_CONTENT_ID];
	$detail[$TAG_NURSE_ID]			= $row[$CONTENT_LIKE_LOGIN_USER_ID];
	$detail[$TAG_STATUS]			= $row[$CONTENT_LIKE_STATUS];
	$detail[$TAG_ENTRY_TIME]		= $row[$CONTENT_LIKE_ENTRY_TIME];
	
	//FETCH NURSE DETAIL
	$_nurse_id		= $row[$CONTENT_LIKE_LOGIN_USER_ID];
	$detail[$TAG_USER] = getNurse($db_con, $_nurse_id);
...
function getContentRow($db_con,$row, $_user_id) {
	...
	$detail[$TAG_LAST_LIKE] = getContentLastLike($db_con, $_content_id);
...
function getClassRow($db_con,$row) {
    ...
    $detail[$TAG_NURSE_ID] 		= $row[$CCP_ATTENDANCE_LOGIN_USER_ID];
    ...
    $detail[$TAG_WARD]     = $row[$CCP_ATTENDANCE_WARD];
    $detail[$TAG_NOTES]     = $row[$CCP_ATTENDANCE_NOTES];
    $detail[$TAG_SESSION_CONDUCTED]     = $row[$CCP_ATTENDANCE_SESSION_CONDUCTED];
...
/* getTrainingLanguage
*/
function getTrainingLanguageRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$ONLINE_TRAINING_LANGUAGE_ID];
    $detail[$TAG_NAME] 				= $row[$ONLINE_TRAINING_LANGUAGE_NAME];
    $detail[$TAG_STATUS] 			= $row[$ONLINE_TRAINING_LANGUAGE_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$ONLINE_TRAINING_LANGUAGE_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$ONLINE_TRAINING_LANGUAGE_EDIT_TIME];
	return $detail;
}	

/* getTrainingCourses
*/
function getTrainingCoursesRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$ONLINE_TRAINING_COURSES_ID];
    $detail[$TAG_LANGUAGE_ID] 	= $row[$ONLINE_TRAINING_COURSES_LANGUAGE_ID];
    $detail[$TAG_NAME] 				= $row[$ONLINE_TRAINING_COURSES_NAME];
    $detail[$TAG_TRAINING_URL] 	= $row[$ONLINE_TRAINING_COURSES_TRAINING_URL];
    $detail[$TAG_STATUS] 			= $row[$ONLINE_TRAINING_COURSES_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$ONLINE_TRAINING_COURSES_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$ONLINE_TRAINING_COURSES_EDIT_TIME];
	
	$_language_id = $row[$ONLINE_TRAINING_COURSES_LANGUAGE_ID];;
    $detail[$TAG_LANGUAGE] 	= getLanguageName($db_con, $_language_id);
...
/* getCCPLetsPlayRow
*/
function getCCPLetsPlayRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_LETS_PLAY_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_LETS_PLAY_NAME];
    $detail[$TAG_URL] 					= $row[$CCP_LETS_PLAY_URL];
    $detail[$TAG_STATUS] 			= $row[$CCP_LETS_PLAY_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_LETS_PLAY_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_LETS_PLAY_EDIT_TIME];
	return $detail;
}	
...
/* getCCPToolTypeRow
*/
function getCCPToolTypeRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_TOOL_TYPE_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_TOOL_TYPE_NAME];
    $detail[$TAG_TYPE] 			= $row[$CCP_TOOL_TYPE_TYPE];
    $detail[$TAG_URL] 				= $row[$CCP_TOOL_TYPE_URL];
    $detail[$TAG_STATUS] 			= $row[$CCP_TOOL_TYPE_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_TOOL_TYPE_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_TOOL_TYPE_EDIT_TIME];
	return $detail;
}	


/* getCCPToolMaterialRow
*/
function getCCPToolMaterialRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_TOOL_MATERIAL_ID];
    $detail[$TAG_TYPE_ID] 			= $row[$CCP_TOOL_MATERIAL_TYPE_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_TOOL_MATERIAL_NAME];
    $detail[$TAG_URL] 					= $row[$CCP_TOOL_MATERIAL_URL];
    $detail[$TAG_TOGGLE] 			= $row[$CCP_TOOL_MATERIAL_TYPE];
    $detail[$TAG_STATUS] 			= $row[$CCP_TOOL_MATERIAL_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_TOOL_MATERIAL_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_TOOL_MATERIAL_EDIT_TIME];
	
	$_type_id = $row[$CCP_TOOL_MATERIAL_TYPE_ID];;
    $detail[$TAG_TYPE] 	= getMaterialType($db_con, $_type_id);
...
/* getMaterialTypeRow
*/
function getMaterialTypeRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_MATERIAL_TYPE_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_MATERIAL_TYPE_NAME];
    $detail[$TAG_STATUS] 			= $row[$CCP_MATERIAL_TYPE_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_MATERIAL_TYPE_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_MATERIAL_TYPE_EDIT_TIME];
	return $detail;
}



/* getMaterialLanguageRow
*/
function getMaterialLanguageRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_MATERIAL_LANGUAGE_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_MATERIAL_LANGUAGE_NAME];
    $detail[$TAG_AVAILABLE] 		= $row[$CCP_MATERIAL_LANGUAGE_AVAILABLE];
    $detail[$TAG_STATUS] 			= $row[$CCP_MATERIAL_LANGUAGE_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_MATERIAL_LANGUAGE_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_MATERIAL_LANGUAGE_EDIT_TIME];
	return $detail;
}



/* getMaterialCategoryRow
*/
function getMaterialCategoryRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_MATERIAL_CATEGORY_ID];
    $detail[$TAG_NAME] 				= $row[$CCP_MATERIAL_CATEGORY_NAME];
    $detail[$TAG_AVAILABLE] 		= $row[$CCP_MATERIAL_CATEGORY_AVAILABLE];
    $detail[$TAG_STATUS] 			= $row[$CCP_MATERIAL_CATEGORY_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_MATERIAL_CATEGORY_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_MATERIAL_CATEGORY_EDIT_TIME];
	return $detail;
}
...
/* getMaterialDataRow
*/
function getMaterialDataRow($db_con,$row) {
	
	include("../include/constant.php");		
	$detail = array();	

    $detail[$TAG_ID] 						= $row[$CCP_MATERIAL_DATA_ID];
    $detail[$TAG_TYPE_ID] 				= $row[$CCP_MATERIAL_DATA_TYPE_ID];
    $detail[$TAG_LANGUAGE_ID] 				= $row[$CCP_MATERIAL_DATA_LANGUAGE_ID];
    $detail[$TAG_CATEGORY_ID] 				= $row[$CCP_MATERIAL_DATA_CATEGORY_ID];
    $detail[$TAG_URL] 				= $row[$CCP_MATERIAL_DATA_MATERIAL_URL];
    $detail[$TAG_AVAILABLE] 		= $row[$CCP_MATERIAL_DATA_AVAILABLE];
    $detail[$TAG_STATUS] 			= $row[$CCP_MATERIAL_DATA_STATUS];
    $detail[$TAG_ENTRY_TIME] 	= $row[$CCP_MATERIAL_DATA_ENTRY_TIME];
    $detail[$TAG_EDIT_TIME] 		= $row[$CCP_MATERIAL_DATA_EDIT_TIME];
	return $detail;
}
...
/* getNurseStatus
*/
function getNurseStatus($db_con,$_nurse_id) {
...
/* getNurseHospitalId
*/
function getNurseHospitalId($db_con,$_nurse_id) {
...
/* getNurseStateId
*/
function getNurseStateId($db_con,$_nurse_id) {
	...
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row = mysqli_fetch_array($result);
			$_district_id				= $row[$NURSE_DISTRICT_ID];
			$_state_id 		= getStateId($db_con,$_district_id);
			return $_state_id;
		}
	}	
	return 0;
...
/* getNurseDistrictId
*/
function getNurseDistrictId($db_con,$_nurse_id) {
...
function getLocation($db_con,$_city_id) {
	...
	$query = "SELECT * FROM `$DISTRICT_TABLE` WHERE `$DISTRICT_ID` = '$_city_id'";
	$result = mysqli_query($db_con,$query);
	...
	if($result){
		if(mysqli_num_rows($result) != 0){
			...
			$detail[$TAG_ID]			= $row[$DISTRICT_ID];
			$detail[$TAG_CITY_NAME]		= $row[$DISTRICT_NAME];//$row[$CITY_NAME];
			$detail[$TAG_DISTRICT_NAME]	= $row[$DISTRICT_NAME];
...
function getStateName($db_con,$_city_id) {
	...
	$query = "SELECT * FROM `$STATE_TABLE` WHERE `$STATE_ID` IN (SELECT `$DISTRICT_STATE_ID` FROM `$DISTRICT_TABLE` WHERE `$DISTRICT_ID` = '$_city_id')";
	$result = mysqli_query($db_con,$query);
...
/* getStateId
*/
function getStateId($db_con,$_city_id) {
	
	include("../include/constant.php");		
	$name = "";
	$query = "SELECT `$DISTRICT_STATE_ID` FROM `$DISTRICT_TABLE` WHERE `$DISTRICT_ID` = '$_city_id'";
	$result = mysqli_query($db_con,$query);
	
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row = mysqli_fetch_array($result);						
			$id		= $row[$DISTRICT_STATE_ID];			
			return $id;
		}
	}	
	return 0;
...
function getCountryName($db_con,$_city_id) {
	...
	$query = "SELECT * FROM `$COUNTRY_TABLE` WHERE `$COUNTRY_ID` IN (SELECT `$STATE_COUNTRY_ID` FROM `$STATE_TABLE` WHERE `$STATE_ID` IN (SELECT `$DISTRICT_STATE_ID` FROM `$DISTRICT_TABLE` WHERE `$DISTRICT_ID` = '$_city_id'))";
	$result = mysqli_query($db_con,$query);
...
*/
function getHospitalCondition($db_con,$_hospital_condition_ids) {
	
	include("../include/constant.php");		
	$detail = array();						
	
	
	$query = "SELECT * FROM `$HOSPITAL_MEDICAL_CONDITION_TABLE` WHERE `$HOSPITAL_MEDICAL_CONDITION_ID` in ($_hospital_condition_ids)";
	$result = mysqli_query($db_con,$query);
...
/* getHospitalPartner
*/
function getHospitalPartner($db_con,$_hospital_id) {
	...
	if($result){
		while ($row = mysqli_fetch_array($result)) {
			$detail = getHospitalPartnerRow($db_con, $row);
			array_push($response, $detail);
		}
	}	
	return $response;
...
/* getLanguageName
*/
function getLanguageName($db_con,$_language_id) {
	
	include("../include/constant.php");		
	$name = "";
	$query = "SELECT * FROM `$ONLINE_TRAINING_LANGUAGE_TABLE` WHERE `$ONLINE_TRAINING_LANGUAGE_ID`  = '$_language_id'";
	$result = mysqli_query($db_con,$query);
	
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row = mysqli_fetch_array($result);						
			$name		= $row[$ONLINE_TRAINING_LANGUAGE_NAME];			
			return $name;
		}
	}	
	return $name;
...
/* getMaterialType
*/
function getMaterialType($db_con,$_language_id) {
	
	include("../include/constant.php");		
	$name = "";
	$query = "SELECT * FROM `$CCP_TOOL_TYPE_TABLE` WHERE `$CCP_TOOL_TYPE_ID`  = '$_language_id'";
	$result = mysqli_query($db_con,$query);
	
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row = mysqli_fetch_array($result);						
			$name		= $row[$CCP_TOOL_TYPE_NAME];			
			return $name;
		}
	}	
	return $name;
...
/* getContentLastLike
*/
function getContentLastLike($db_con,$_content_id) {
	
	include("../include/constant.php");		
	$detail = array();							
	$query = "SELECT * FROM `$CONTENT_LIKE_TABLE` WHERE `$CONTENT_LIKE_CONTENT_ID` = '$_content_id' AND `$CONTENT_LIKE_STATUS` = '$STATUS_ACTIVE' ORDER BY `$CONTENT_LIKE_ENTRY_TIME` ASC";
	$result = mysqli_query($db_con,$query);
	
	if($result){
		if(mysqli_num_rows($result) != 0){
			$row = mysqli_fetch_array($result);
			$detail = getLikeRow($db_con,$row);
			return $detail;
		}
	}
	return null;
...
function isUserContent($db_con,$_content_id, $_user_id) {
	...
	$query = "SELECT * FROM `$CONTENT_TABLE` WHERE `$CONTENT_ID` IN (SELECT `$CONTENT_GROUP_CONTENT_ID` FROM `$CONTENT_GROUP_TABLE` WHERE `$CONTENT_GROUP_GROUP_ID` IN (SELECT `$GROUP_USER_GROUP_ID` FROM `$GROUP_USER_TABLE` WHERE `$GROUP_USER_USER_ID` = '$_user_id') OR `$CONTENT_GROUP_GROUP_ID` IN (SELECT `$GROUP_ID` FROM `$GROUP_TABLE` WHERE `$GROUP_GROUP_MEMBERS` = '1') OR `$CONTENT_GROUP_GROUP_ID` = '0') AND `$CONTENT_STATUS` = '$STATUS_ACTIVE' AND `$CONTENT_ID` = '$_content_id'";
	$result = mysqli_query($db_con,$query);
...
function sendOTPMessage($db_con,$_user_id) {
	...
	if($_send_sms){
		...
		if($result){
			...
			$otpExpiry = 5;
			$otpExpiryUnit = "mins";
			$otpSender = "Care Companion";
			
			$otpMessage = "The OTP for $otpType is $_otp. This OTP is valid for next $otpExpiry $otpExpiryUnit - $otpSender Team.";
			...
			$_postResult = json_encode($postResult);
			$_mobile_number =  $nurse[$TAG_MOBILE_NUMBER];
			
			
			$queryPost	= "INSERT INTO `noora_user_mobile_attempt_data` (`mobile`, `post_data`, `post_response`, `$USER_MOBILE_ATTEMPT_STATUS`, `$USER_MOBILE_ATTEMPT_ENTRY_TIME`, `$USER_MOBILE_ATTEMPT_EDIT_TIME`) VALUES ('$_mobile_number', '$_postData', '$_postResult', '$STATUS_ACTIVE', '$dateNow', '$dateNow')";
			$resultPost = mysqli_query($db_con,$queryPost);		
			...
			$response["error_msg"] = "Failed to insert or update in USER OTP TABLE";
			$response["error_q"] = "$query";
			//$response["error_sms"] = "$postResult";
...
/* updateNurseHistory
*/
function updateNurseHistory($db_con,$_user_id, $_type_id, $_content_id, $_entryTime, $dateNow, $_sessionId) {
	
	//$_sessionId="xxx";
	
	include("../include/constant.php");			
	$query = "INSERT INTO `$NURSE_HISTORY_TABLE` (`$NURSE_HISTORY_NURSEID`, `$NURSE_HISTORY_HISTORY_TYPE_ID`, `$NURSE_HISTORY_CONTENT_ID`, `$NURSE_HISTORY_ENTRY_TIME`, `$NURSE_HISTORY_EDIT_TIME`, `$NURSE_HISTORY_SESSION_ID`) VALUES ('$_user_id', '$_type_id', '$_content_id', '$_entryTime', '$dateNow', '$_sessionId');";
...
$app->post('/getAppSetting', function() use ($app){
	...
	$result = authenticateForRole($db_con, $app, false);
	if($result[$TAG_SUCCESS]){
		mysqli_set_charset($db_con,'utf8');	
	...
	if($result[$TAG_SUCCESS]){
		...
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				$message = "You will need to register this number before you can login. Please contact the CCP Program Manager for more information.";
				$addNewUser = True;
			}
...
$app->post('/setContentLike', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		//FOR SESSION HANDLE 		
		if($_user_id != 0){
			$user_status = getNurseStatus($db_con, $_user_id);
			if ($user_status == $STATUS_ACTIVE) {
				//CODE
				$query	= "SELECT * FROM `$CONTENT_LIKE_TABLE` WHERE `$CONTENT_LIKE_CONTENT_ID` = '$_content_id' AND `$CONTENT_LIKE_LOGIN_USER_ID` = '$_user_id'";
				$result = mysqli_query($db_con,$query);
...
$app->post('/setContentLikeBulk', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		// foreach($array as $item) {
			...
			// $_status_val 		= $item[$TAG_STATUS];
			// $_entryTime		= $item[$TAG_ENTRY_TIME];
			// //$_sessionId		= $item[$TAG_SESSION_ID];
			// $_sessionId 	= getval($db_con,$item,$TAG_SESSION_ID);

			$_user_id 		= getval($db_con,$data,$TAG_USER_ID);
			$_content_id	= getval($db_con, $data, $TAG_CONTENT_ID);
			$_status_val	= getval($db_con, $data, $TAG_STATUS);
			$_entryTime 	= getval($db_con,$data,$TAG_ENTRY_TIME);
			$_sessionId 	= getval($db_con,$data,$TAG_SESSION_ID);
			...
			if($_user_id != 0){
				...
				if ($user_status == $STATUS_ACTIVE) {
					...
					if($_status_val){ //Only when liked
						// Track Nurse Content like/unlike
						updateNurseHistory($db_con,$_user_id, $HISTORY_TYPE_CONTENT_LIKE, $like_id, $_entryTime, $dateNow, $_sessionId);
		...
		$response[$TAG_SUCCESS] = True;
		$response[$TAG_MESSAGE] = "Slots Updated ".$query;
		echoRespnse(200, $response, $key);	
	}
...
$app->post('/getContentComment', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		//CODE
		//$query	= "SELECT * FROM `$CONTENT_COMMENT_TABLE` WHERE `$CONTENT_COMMENT_CONTENT_ID` = '$_content_id' AND `$CONTENT_COMMENT_STATUS` = '$STATUS_ACTIVE' ORDER BY `$CONTENT_COMMENT_ENTRY_TIME` DESC";
		$query	= "SELECT * FROM `$CONTENT_COMMENT_TABLE` WHERE `$CONTENT_COMMENT_CONTENT_ID` = '$_content_id' ORDER BY `$CONTENT_COMMENT_ENTRY_TIME` DESC";
		$result = mysqli_query($db_con,$query);
 ...
 * params - TAG_USER_ID
 */
$app->post('/getContentLikes', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		//CODE
		$query	= "SELECT * FROM `$CONTENT_LIKE_TABLE` WHERE `$CONTENT_LIKE_CONTENT_ID` = '$_content_id'  AND `$CONTENT_LIKE_STATUS` = '$STATUS_ACTIVE' ORDER BY `$CONTENT_LIKE_ENTRY_TIME` DESC";
		$result = mysqli_query($db_con,$query);
	...
	if($result[$TAG_SUCCESS]){
		...
		if($_user_id != 0){
			...
			if ($user_status == $STATUS_ACTIVE) {
				...
				updateNurseHistory($db_con,$_user_id, $HISTORY_TYPE_CONTENT_COMMENT, $comment_id, $dateNow, $dateNow, $_sessionId);
...
$app->post('/postContentCommentBulk', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		// foreach($array as $item) { 
			$_user_id 			= getval($db_con,$data,$TAG_USER_ID);
			$_content_id 		= getval($db_con,$data,$TAG_CONTENT_ID);
			$_comment			= getval1($db_con,$data,$TAG_COMMENT);
			$_entryTime			= getval($db_con,$data,$TAG_ENTRY_TIME);
			$_sessionId			= getval($db_con,$data,$TAG_SESSION_ID);
			$_mentioned_user_id	= getval($db_con,$data,$TAG_MENTIONED_USER_ID);
			...
			if($_user_id != 0){
				...
				if ($user_status == $STATUS_ACTIVE) {
					//CODE
					$query	= "INSERT INTO `$CONTENT_COMMENT_TABLE` (`$CONTENT_COMMENT_CONTENT_ID`, `$CONTENT_COMMENT_COMMENT`, `$CONTENT_COMMENT_LOGIN_USER_ID`, `$CONTENT_COMMENT_ENTRY_TIME`, `$CONTENT_COMMENT_EDIT_TIME`,`$TAG_MENTIONED_USER_ID`) VALUES ('$_content_id','$_comment','$_user_id','$_entryTime','$dateNow','$_mentioned_user_id')";
					$result = mysqli_query($db_con,$query);	
					if($result){	
						$comment_id = mysqli_insert_id($db_con);
					}else{
						$_log_query = $query;
						$_log_query = $_log_query." Error description: " . mysqli_error($db_con);				
					}

					// Track Nurse Content comment
					updateNurseHistory($db_con,$_user_id, $HISTORY_TYPE_CONTENT_COMMENT, $comment_id, $_entryTime, $dateNow, $_sessionId);
			...
			if ($_mentioned_user_id !='') {
				sendMentionNotification($db_con,$_mentioned_user_id,$comment_id);
			}			
		// }
		
		$response[$TAG_SUCCESS] = True;
		$response[$TAG_MESSAGE] = "Slots Updated ".$_log_query;
		echoRespnse(200, $response, $key);	
	}
...
* headers - token
*/
$app->post('/getNurseFullProfile', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				while ($row = mysqli_fetch_array($result)) {
					$nurse = getNurseRow($db_con, $row);
					$profile = getNurseProfileRow($db_con, $row);
					$detail = array();
					$detail[$TAG_NURSE] = $nurse;
					$detail[$TAG_NURSE_PROFILE] = $profile;
					$response[$TAG_DETAILS]= $detail;
				}
			}
		}
...
* url - /getTrainingLanguage
* headers - token
*/
$app->post('/getTrainingLanguage', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$response[$TAG_DETAILS] = array();
		$success = false;
		$message = "No Course Languages found";
		
		$query = "SELECT * FROM `$ONLINE_TRAINING_LANGUAGE_TABLE`";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				$success = true;
				$message = "Course Languages found Successfully.";
				
				while ($row = mysqli_fetch_array($result)) {
					$detail = getTrainingLanguageRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}
			}
		}
...
/** Get Training Course
* url - /getTrainingCourses
* headers - token
*/
$app->post('/getTrainingCourses', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$query = "SELECT * FROM `$ONLINE_TRAINING_COURSES_TABLE`";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				while ($row = mysqli_fetch_array($result)) {
					$detail = getTrainingCoursesRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}
			}
		}
...
$app->post('/submitClass', function() use ($app){
	...
	if($result[$TAG_SUCCESS]){
		...
		$_user_id 		= getval($db_con,$data,$TAG_USER_ID);
		$_class_id	= getval($db_con, $data, $TAG_CLASS_ID);
        ...
        $_ward     = getval($db_con,$data,$TAG_WARD);
        $_notes     = getval($db_con,$data,$TAG_NOTES);
        $_session_conducted     = getval($db_con,$data,$TAG_SESSION_CONDUCTED);
		...
		if($_user_id != 0){
			...
			if ($user_status == $STATUS_ACTIVE) {
				...
				//GET NURSE STATE AND DISTRICT IDS
				$_district_id = getNurseDistrictId($db_con, $_user_id);
				$_state_id = getNurseStateId($db_con, $_user_id);
				$_hospital_id = getNurseHospitalId($db_con, $_user_id);
				
				if($_class_id != 0){
					
					if(!empty($_image_name)){
							$queryImage = "UPDATE `$CCP_ATTENDANCE_TABLE` SET `$CCP_ATTENDANCE_IMAGE` = '$_image_name', `$CCP_ATTENDANCE_EDIT_TIME`= '$dateNow' WHERE `$CCP_ATTENDANCE_ID` = '$_class_id'";
							$resultImage = mysqli_query($db_con, $queryImage);
					}
					
					$query = "UPDATE `$CCP_ATTENDANCE_TABLE` SET `$CCP_ATTENDANCE_CLASS_TYPE_ID` = '$_class_type', `$CCP_ATTENDANCE_CLASS_DATE` = '$_class_date', `$CCP_ATTENDANCE_CLASS_TIME` =  '$_class_time', `$CCP_ATTENDANCE_NO_OF_PEOPLE` =  '$_no_of_people', `$CCP_ATTENDANCE_NO_OF_FAMILY` = '$_no_of_family', `$CCP_ATTENDANCE_WARD` = '$_ward', `$CCP_ATTENDANCE_NOTES` = '$_notes', `$CCP_ATTENDANCE_SESSION_CONDUCTED` = '$_session_conducted', `$CCP_ATTENDANCE_STATE_ID` = '$_state_id', `$CCP_ATTENDANCE_DISTRICT_ID` = '$_district_id', `$CCP_ATTENDANCE_HOSPITAL_ID` = '$_hospital_id',  `$CCP_ATTENDANCE_EDIT_TIME`= '$dateNow' WHERE `$CCP_ATTENDANCE_ID` = '$_class_id'";
					$result = mysqli_query($db_con,$query);
					$ccp_id = $_class_id;
						$success = true;
						$message = "Ticket Updated Successfully.".$query;		
				}else{	
					$query = "INSERT INTO `$CCP_ATTENDANCE_TABLE` (`$CCP_ATTENDANCE_LOGIN_USER_ID`, `$CCP_ATTENDANCE_CLASS_TYPE_ID`, `$CCP_ATTENDANCE_IMAGE`, `$CCP_ATTENDANCE_CLASS_DATE`, `$CCP_ATTENDANCE_CLASS_TIME`, `$CCP_ATTENDANCE_NO_OF_PEOPLE`, `$CCP_ATTENDANCE_NO_OF_FAMILY`, `$CCP_ATTENDANCE_WARD`, `$CCP_ATTENDANCE_NOTES`, `$CCP_ATTENDANCE_SESSION_CONDUCTED`, `$CCP_ATTENDANCE_STATE_ID`, `$CCP_ATTENDANCE_DISTRICT_ID`, `$CCP_ATTENDANCE_HOSPITAL_ID`, `$CCP_ATTENDANCE_STATUS`, `$CCP_ATTENDANCE_ENTRY_TIME`, `$CCP_ATTENDANCE_EDIT_TIME`) VALUES ('$_user_id', '$_class_type', '$_image_name', '$_class_date', '$_class_time', '$_no_of_people', '$_no_of_family', '$_ward', '$_notes', '$_session_conducted', '$_state_id', '$_district_id', '$_hospital_id', '$STATUS_ACTIVE', '$_entryTime', '$dateNow');";
					$result = mysqli_query($db_con,$query);
					if($result){
				...
				updateNurseHistory($db_con,$_user_id, $HISTORY_TYPE_CLASS_ADD, $ccp_id, $_entryTime, $dateNow, $_sessionId);
...
$app->post('/updateUserSession', function() use ($app) {
	...
	if($result[$TAG_SUCCESS]){
		...
		// foreach($array as $item) { 
			...
			updateNurseHistory($db_con,$_user_id, $_historyType, $_user_id, $_entryTime, $dateNow, $_sessionId);
...
$app->post('/updateUserProfile', function() use ($app){
	...
	if($result[$TAG_SUCCESS]){
		...
		$_hospital_name 		= getval($db_con,$data,$TAG_HOSPITAL_NAME);
        ...
        $_ccp_condition_id            = getval($db_con,$data,$TAG_CCP_CONDITION_ID);
        $_designation             = getval($db_con,$data,$TAG_DESIGNATION);
        $_trainer             = getval($db_con,$data,$TAG_TRAINER);
		...
		if($_hospital_id == -1){
			...
			$query = "INSERT INTO `$HOSPITAL_TABLE` (`$HOSPITAL_NAME`, `$HOSPITAL_STATUS`, `$HOSPITAL_ENTRY_TIME`, `$HOSPITAL_EDIT_TIME`) VALUES ('$_hospital_name', '$STATUS_ACTIVE', '$dateNow', '$dateNow');";
			$result = mysqli_query($db_con,$query);
			if($result) {
				$hospitalId = mysqli_insert_id($db_con);
				
				$query = "UPDATE `$NURSE_TABLE` SET `$NURSE_HOSPITAL_ID` = '$hospitalId', `$NURSE_EDIT_TIME` = '$dateNow' WHERE `$NURSE_ID` = '$_user_id'";
				$result = mysqli_query($db_con,$query);						
			}
		...
		$query = "UPDATE `$NURSE_TABLE` SET `$NURSE_FIRST_NAME` = '$_fname', `$NURSE_LAST_NAME` = '$_lname', `$NURSE_DOB` = '$_dob', `$NURSE_GRADUATING_YEAR` = '$_graduation', `$NURSE_HOSPITAL_JOINING_DATE` = '$_joining_date', `$NURSE_DESIGNATION` = '$_designation', `$NURSE_CCP_CONDITION_ID` = '$_ccp_condition_id', `$NURSE_TRAINER` = '$_trainer', `$NURSE_TOT_DATE` = '$_tot_date', `$NURSE_EDIT_TIME` = '$dateNow' WHERE `$NURSE_ID` = '$_user_id'";
		$result = mysqli_query($db_con,$query);	
...
/** Get CCP Lets Play list
* url - /getCCPLetsPlay
* headers - token
*/
$app->post('/getCCPLetsPlay', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$query = "SELECT * FROM `$CCP_LETS_PLAY_TABLE`";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				$success = true;
				$message = "CCP Games found Successfully.";
				
				while ($row = mysqli_fetch_array($result)) {
					$detail = getCCPLetsPlayRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}
			}
		}
...
* headers - token
*/
$app->post('/getCCPToolType', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$query = "SELECT * FROM `$CCP_TOOL_TYPE_TABLE`";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				while ($row = mysqli_fetch_array($result)) {
					$detail = getCCPToolTypeRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}
			}
		}
...
/** Get CCP Tool Material List
* url - /getCCPToolMaterial
* headers - token
*/
$app->post('/getCCPToolMaterial', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$query = "SELECT * FROM `$CCP_TOOL_MATERIAL_TABLE`";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				while ($row = mysqli_fetch_array($result)) {
					$detail = getCCPToolMaterialRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}
			}
		}
...
* headers - token
*/
$app->post('/getNurseName', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$_nurse_id 			= getval($db_con, $data, $TAG_NURSE_ID);
		$_content_date 		= getval($db_con, $data, $TAG_UPDATED_DATE);
		$_search_term		= getval($db_con, $data, $TAG_NURSE_SEARCH);
		...
		if ($_search_term == '' ) {
			$query = "SELECT * FROM `$NURSE_TABLE` ORDER BY RAND() LIMIT 10  ";
			...
			if ($result) {
				if(mysqli_num_rows($result) != 0){
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						$detail['id'] =$row[$NURSE_ID];
						$detail['name'] =$row[$NURSE_FIRST_NAME];
						$detail['image'] =$row[$NURSE_PROFILE_IMAGE];
						array_push($response[$TAG_DETAILS], $detail);
					}
				}
			}	
		} elseif ($_search_term != '' ) {
			$query = "SELECT * FROM `$NURSE_TABLE` WHERE First_Name LIKE '".$_search_term."%'  ";
			$result = mysqli_query($db_con,$query);
			if ($result) {
				if(mysqli_num_rows($result) != 0){
					$success = true;
...
* headers - token
*/
$app->post('/getNurseList', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		if($_content_date != ''){
			$_latestQuery = " AND `$NURSE_EDIT_TIME` > '$_content_date'";	
		}
		$response[$TAG_UPDATED_DATE] = $_content_date;

		$query = "SELECT * FROM `$NURSE_TABLE` WHERE 1=1 $_latestQuery ORDER BY `$NURSE_EDIT_TIME` DESC";
		$result = mysqli_query($db_con,$query);				
		if($result){
			...
			if(mysqli_num_rows($result) != 0){
				...
				while ($row = mysqli_fetch_array($result)) {	
					if($contentDate == ''){
						$contentDate = $row[$NURSE_EDIT_TIME];
					}
					$detail = getNurseUserRow($db_con, $row);
					array_push($response[$TAG_DETAILS], $detail);
				}	
				$response[$TAG_UPDATED_DATE] = $contentDate;			
			}
...
/** Get CCP Material Data
* url - /getNurseList
* headers - token
*/
$app->post('/getMaterialMasterData', function() use ($app){
	// check for required params
	include("../include/constant.php");
	$db = new DB_CONNECT();
	$db_con=$db->conn;
	...
	if($result[$TAG_SUCCESS]){
		...
		$detail = array();
		$materialTypes = array();
		$materialLanguagess = array();
		$materialCategories = array();
		$materialData = array();

		//1. Get Material Types
		$query	= "SELECT * FROM `$CCP_MATERIAL_TYPE_TABLE` ";
		$result = mysqli_query($db_con,$query);				
		if($result){
			if(mysqli_num_rows($result) != 0){
				while ($row = mysqli_fetch_array($result)) {				
					$item = getMaterialTypeRow($db_con, $row);
					array_push($materialTypes, $item);
				}	
			}
		}
		
		//2. Get Material Languages
		$query	= "SELECT * FROM `$CCP_MATERIAL_LANGUAGE_TABLE` ";
		$result = mysqli_query($db_con,$query);				
		if($result){
			if(mysqli_num_rows($result) != 0){
				while ($row = mysqli_fetch_array($result)) {
					$item = getMaterialLanguageRow($db_con, $row);
					array_push($materialLanguagess, $item);
				}	
			}
		}
		
		//3. Get Material Categories
		$query	= "SELECT * FROM `$CCP_MATERIAL_CATEGORY_TABLE` ";
		$result = mysqli_query($db_con,$query);				
		if($result){
			if(mysqli_num_rows($result) != 0){
				while ($row = mysqli_fetch_array($result)) {				
					$item = getMaterialCategoryRow($db_con, $row);
					array_push($materialCategories, $item);
				}	
			}
		}
		
		//4. Get Material Data
		$query	= "SELECT * FROM `$CCP_MATERIAL_DATA_TABLE` ";
		$result = mysqli_query($db_con,$query);				
		if($result){
			if(mysqli_num_rows($result) != 0){
				while ($row = mysqli_fetch_array($result)) {				
					$item = getMaterialDataRow($db_con, $row);
					array_push($materialData, $item);
				}	
			}
		}
		
		$detail[$TAG_MATERIAL_TYPE] = $materialTypes;	
		$detail[$TAG_MATERIAL_LANGUAGE] = $materialLanguagess;		
		$detail[$TAG_MATERIAL_CATEGORY] = $materialCategories;	
		$detail[$TAG_MATERIAL_DATA] = $materialData;		
