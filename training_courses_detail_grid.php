...
$query1 = "SELECT " . $table_noora_online_training_courses . ".*  FROM " . $table_noora_online_training_courses . " WHERE 1=1 AND " . $table_noora_online_training_courses . ".Status = '1' ";
$res1 = mysqli_query($link, $query1);
$totalData = mysqli_num_rows($res1);
$query1 = "SELECT " . $table_noora_online_training_courses . ".*," . $table_noora_online_training_language . ".Name as Language_Name FROM " . $table_noora_online_training_courses . " INNER JOIN " . $table_noora_online_training_language . " ON " . $table_noora_online_training_courses . ".Language_ID = " . $table_noora_online_training_language . ".ID WHERE 1=1 AND " . $table_noora_online_training_courses . ".Status = '1' ";
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
   // $query1 .= " AND ( " . $table_noora_online_training_courses . ".Name LIKE '%" . $requestData['search']['value'] . "%' )";
	
    $query1 .= " AND ( " . $table_noora_online_training_courses . ".Name LIKE '%" . $requestData['search']['value'] . "%'  OR ".$table_noora_online_training_language .".Name LIKE '%" . $requestData['search']['value'] . "%' )";
}
$res1 = mysqli_query($link, $query1);
...
while ($row1 = mysqli_fetch_array($res1)) {  // preparing an array
    ...
    $nestedData[] = $Language;
    $nestedData[] = $Course;
    $nestedData[] = "<a href='" . $Training_URL . "'>$Training_URL</a>";
    $nestedData[] = $Status;
